# 动漫高手 · 一眼顶针
[游戏链接](https://davy-chendy.github.io/anime-master-game/)

## 引言

todo

## 玩法介绍

完整图片被分割并遮挡，

主持人逐步揭开部分区域，玩家根据当前画面猜出对应的动画作品，

展示区域会随轮次逐渐增多，直到有人猜中为止。

### 示例

初始只显示少量区域：
![示例1](./figures/exp1.png)

多轮后逐渐揭开：
![示例2](./figures/exp2.png)

## 游戏前配置

最高配置：**朋友**

- 需要指定一名玩家作为 **主持人**
  
   主持人在电脑上打开游戏，并将「玩家屏」窗口 **投屏 / 屏幕共享** 给所有答题者（如腾讯会议共享）。
   
   主持人负责操作揭示方块与切换题目。
   
- **投屏软件**（有其它更好的软件欢迎分享
  
   * 腾讯会议，缺点是人多会有40min限时
   * kook，需要开会员才能共享
   * 觅讯，有网页版，看起来是不限时长的腾讯会议
   
- **⚠️共享屏幕注意事项**
  
  - 请勿最小化玩家屏
  - 请勿让主持人屏以最大化状态完全遮挡玩家屏
  - 建议主持人屏与玩家屏均使用 **窗口化模式**，以保证画面能够实时刷新
  
  

![投屏示例](./figures/投屏示例.png)






## 🖼 如何把本地图片批量转成可用链接（图床方式）

本游戏题库需要使用**图片链接（URL）**。 如果只有本地图片，请先用图床转换成链接。

**注意**

* 图片大小尽量一致，尽量是16 : 9的样式
* 推荐直接截图，不用特意找超清图，以免加载过慢

以 **ImgSea 图床（ https://imgsea.com ）** 为

（最初随手找的免费图床，永久存储但**广告多**，有更好的图床欢迎推荐！）

**其它免费图床** （作者也不清楚哪个更好用，可以都试试

* https://www.picgo.net/（无广告）
* https://pnglog.com/ （无广告，免费5G空间）
* ...

### 第一步：上传图片

1. 打开：https://imgsea.com   注册并登录账号  
3. 上传你的图片（支持多选）

### 第二步：获取嵌入代码

进入个人主页后：

1. **全选图片** (仅当前页的全部24张图片，超过24张需翻页)
2. 点击：**获取嵌入代码**

或者先把图片移动到相册，相册能一次性获取所有图片的url

![获取嵌入代码](./figures/imgsea1.png)


### 第三步：选择“直接（源文件）链接”

在弹出的选项中选择：

👉 **直接（源文件）链接 / 图片URL链接**

![选择直接源文件链接](./figures/imgsea2.png)

⚠️注意：不是查看链接（此类没有图片后缀，例如https://imgsea.com/image/QXJf ），**一定要是以.jpg .png等结尾**的链接

### 第四步：复制链接使用

你会得到一批图片链接，例如：

```txt
https://imgsea.com/xxxx.jpg
https://imgsea.com/yyyy.png
https://imgsea.com/zzzz.webp
```
复制进游戏即可作为题库





## 规则（自用）
### 基本要求

- 参与人数：5 人
- 每人准备 20 张图片，共 100 张
- 图片范围限定为 **2005 年后日本动画** 的截图
  - 包括日本动画电影
  - 不包括真人剧

### 游戏流程概述

- 游戏共进行 100 题
- 每一题由一名玩家担任 **出题者**，其余玩家为 **答题者**
- 出题者仅负责自己的题库，展示图片与选择遮挡区域，不参与本题抢答

### 出题与展示方式

- 出题者从自己准备的 20 张图片中选择一张作为题目
- 初始时，图片被划分为 **5 × 9 共 45 个大小相同的方块**，全部被遮挡
- 题目将分多轮进行展示，每一轮出题者自行选择 **任意个方块** 进行展示
- 每轮展示完成后，进入抢答阶段
- 若无人抢答，则进入下一轮，继续展示新的方块
- 若 45 个方块全部展示完毕仍无人抢答，则本题结束

### 抢答规则

- 每一轮展示后，设置 **10 秒抢答时间**
- 抢答采取自由抢答制，允许多人在同一轮同时抢答
- 每名答题者在**同一题目**中 **仅有一次抢答机会**

- 所有抢答但**未答对**的玩家，在本题中 **失去后续作答资格**

### 关于“无法猜出”的判定

- 游戏中不存在主动声明“无法猜出”的行为
- 若一名答题者 **直到图片 45 个方块全部展示完毕仍未进行抢答**，则视为该玩家本题“无法猜出”

### 积分规则

- 当 **有且仅有一位答题者首次抢答并回答正确** 时：
  - 该答题者 **+1 分**
  - 出题者 **+1 分**
- 当 **所有答题者都进行抢答，且全部回答错误** 时：
  - 出题者 **+1 分**
- 当 **图片全部展示完毕，且至少有一名答题者未进行抢答** 时：
  - 所有答题者 **各 +1 分**

### 补充说明

- 出题者在本题中不参与抢答与得分判定（仅通过积分规则获得分数）
- 所有得分情况均以题目结束时的状态为准
- 游戏以总积分高低决定最终排名



